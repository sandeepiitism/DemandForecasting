...
#         pred_set = list(self.series_df.values[-self.n_steps_in:])
#         for i in range(0, self.fh):
#             # print(self.n_steps_in, self.n_features, len(pred_set))
#             p = model.predict(
...
#         return pd.DataFrame(np.array(pred_set[-self.fh:]).reshape(1, -1))
    ...
    def predict(self):
        ...
        for i in range(0, self.fh):
            try:
                # Debugging print
                # print("Before prediction:", np.asarray(pred_set[-self.n_steps_in:], dtype='float').shape)

                # p = model.predict(
                #     np.asarray(pred_set[-self.n_steps_in:], dtype='float').reshape((1, self.n_steps_in, self.n_features)).astype('float')).flatten()
                p = float(model.predict(np.asarray(pred_set[-self.n_steps_in:], dtype='float').reshape((1, self.n_steps_in, self.n_features)).astype('float')).flatten()[0])

                # print("After prediction:", p.shape)
                # print(p, type(p))
                ...
                print("Error in prediction for material {}: {}".format(self.detail_df[0], e))
