...
def anomaly_detection(df_org, contamination=0.05, k=1.5):
    ...
    df_com = pd.merge(df_bol, df_if, how='left')
...
def outlier_correction_new(series_df, contamination=0.05, k=1.5):
        ...
        for i in range(len(anomaly_df)):
            ...
            # temp = pd.concat([temp,corrected[(corrected['Date'].dt.month != anomaly_df['Date'].iloc[i].month) & 
            #                                  (corrected['Date'].dt.year == anomaly_df['Date'].iloc[i].year)]], ignore_index=True)
    ...
    #temp1 = temp1[1:]
                ...
                # temp1 = t1.append(t2)
                # Changed in enhancements
                temp1 = pd.concat([t1, t2], ignore_index=True)
                flag=1
...
#     dataset.columns = ['Date','y']
    dataset.columns = ["y"]
#     print('Frequency is ', freq)
    # display(dataset.shape, dataset.columns, dataset)
...
#     dataset['ds'] = pd.to_datetime(dataset.index)#.strftime('%W/%Y') # Changed in enhancements
    ...
    df_prophet['floor'] = 0
    # display(df_prophet.shape, df_prophet.columns, df_prophet)
