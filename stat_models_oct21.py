...
#         series_df = self.series_df[self.series_df!=0]
        ...
        warnings.simplefilter('ignore', ConvergenceWarning)
        model = ExponentialSmoothing(np.asarray(self.series_df), trend=self.trend, seasonal="mul", damped_trend = self.damped, seasonal_periods=self.freq,initialization_method='estimated').fit(optimized=True, remove_bias = self.residual_bias)
...
class Prophet(Prediction_model):
    ...
    def tune(self):
        ...
        df_prophet, _, scaler_to_fix_decimal = prophet_data_preperation(self.series_df, self.cap_power)
#         if(df_prophet.empty):
#             return pd.DataFrame()
...
#         return
        ...
        for params in tqdm(all_params):
            ...
            m = fbprophet.Prophet(**params).fit(df_prophet)  # Fit model with given params
            ...
            df_cv = fbprophet.diagnostics.cross_validation(m, initial='{} days'.format(a.days-365),  horizon='365 days', parallel = 'threads')
...
#         print(tuning_results)
    ...
    def predict(self):
        ...
        m = fbprophet.Prophet(growth=self.growth, changepoint_prior_scale=self.changepoint_prior_scale,seasonality_prior_scale=self.seasonality_prior_scale,daily_seasonality=False, weekly_seasonality='auto',yearly_seasonality='auto',seasonality_mode=self.seasonality_mode)

#         m = fbprophet.Prophet(growth='logistic', changepoint_prior_scale=0.5, daily_seasonality=False, weekly_seasonality=False)
        m.fit(df_prophet)
