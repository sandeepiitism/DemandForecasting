    ...
    def get_model(self):
        ...
        return BayesSearchCV(
            estimator=XGBRegressor(
                objective='reg:squarederror', #changed in enhancements
                booster='gbtree',
                n_jobs=1,
                eval_metric='rmse',  # logloss
                # silent=True, #changed in enhancements
                tree_method='approx',
                # verbose=1, #changed in enhancements
            ),
            ...
            search_spaces={
                ...
                'reg_lambda': (1e-9, 1.0, 'log-uniform'), #(1e-9, 1000, 'log-uniform'), #changed in enhancements
                'reg_alpha': (1e-9, 1.0, 'log-uniform'),
                'gamma': (1e-9, 0.5, 'log-uniform'),
                'min_child_weight': (0, 5),
                'n_estimators': (50, 100),
                # 'scale_pos_weight': (1e-5, 500, 'log-uniform') #changed in enhancements not required because it is mainly for classification task
            },
